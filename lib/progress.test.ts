...
describe('progress', () => {
  ...
  it('markAnswer stores result', () => {
    markAnswer('p3c1_001', 'correct')
    const p = loadProgress()
    expect(p.answers['p3c1_001']).toBe('correct')
  })

  it('toggleBookmark adds and removes', () => {
    toggleBookmark('p3c1_001')
    expect(loadProgress().bookmarks).toContain('p3c1_001')
    toggleBookmark('p3c1_001')
    expect(loadProgress().bookmarks).not.toContain('p3c1_001')
  })
  ...
  it('getStats computes correctly', () => {
    const store: ProgressStore = {
      answers: { 'p3c1_001': 'correct', 'p3c1_002': 'wrong' },
      bookmarks: [],
      lastVisited: null,
      examHistory: [],
    }
    const idMap = { 'p3c1_001': 'part3_ch1', 'p3c1_002': 'part3_ch1' }
    const stats = getStats(store, idMap)
    expect(stats.byChapter['part3_ch1'].correct).toBe(1)
    expect(stats.byChapter['part3_ch1'].attempted).toBe(2)
    expect(stats.byPart[3].correct).toBe(1)
  })
})
