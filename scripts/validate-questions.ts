...
const ID_PATTERN = /^(p[1-3]c[1-3]_\d{3}|exam[12]_\d{3})$/
...
function validateQuestion(q: QuestionRaw, filePath: string): string[] {
  ...
  if (!q.id || typeof q.id !== 'string') {
    ...
    errors.push(`[${file}] id 패턴 불일치: "${q.id}" (기대: p[1-3]c[1-3]_NNN 또는 exam[12]_NNN)`)
  }

  if (q.part === undefined || q.part < 1 || q.part > 3) {
    errors.push(`[${file}][${q.id}] part 범위 오류: ${q.part} (1~3 필요)`)
  }
...
function main() {
  ...
  const countsByPart: Record<number, number> = {}
  ...
  for (const filePath of allFiles) {
    ...
    countsByFile[relFile] = data.length
    ...
    for (const q of data) {
      ...
      if (typeof q.part === 'number') {
        countsByPart[q.part] = (countsByPart[q.part] ?? 0) + 1
      }
    }
  }
  ...
  for (const file of Object.keys(countsByFile).sort()) {
    console.log(`  - ${file}: ${countsByFile[file]}개`)
  }

  console.log(`\n과목별 문항 수:`)
  for (const part of Object.keys(countsByPart).map(Number).sort((a, b) => a - b)) {
    console.log(`  - ${part}과목: ${countsByPart[part]}개`)
  }
